...
const jadibot = async (zynX, m, from) => {
      ...
      client[from].ev.on("connection.update", async up => {
        ...
        if (connection === "close") {
          if (reason === DisconnectReason.restartRequired) {
            ...
            return zynX.sendMessage(from, { text: "Anda sudah tidak lagi menjadi bot." })
          }
        }
      })
...
async function stopjadibot(zynX, from) {
  if (!Object.keys(client).includes(from)) {
    return zynX.sendMessage(from, { text: "Anda tidak ada di list jadi bot." })
  }
  try {
...
async function listjadibot(zynX, m) {
  ...
  return zynX.sendMessage(from, { text: text.trim(), mentions, })
}
